...
import torch
from my_lib.datasets.preprocessing import (inception_preprocess,
                                              rtpose_preprocess,
...
def get_val_dataset(image_dir,anno_file,preprocess):
...
def run_eval(image_dir, anno_file, vis_dir, model, preprocess):
    ...
    for i in range(len(img_ids)):
        ...
        h_ratio = oriImg.shape[0] / 384
        w_ratio = oriImg.shape[1] / 384

        new_ori_img = cv2.resize(oriImg,(384,384))

        # # Get the shortest side of the image (either height or width)
        # shape_dst = np.min(oriImg.shape[0:2])

        # Get results of original image
        paf, heatmap = get_outputs(new_ori_img, model,  preprocess)
        ...
        upsample_keypoints = (heatmap.shape[0]*cfg.MODEL.DOWNSAMPLE*w_ratio, heatmap.shape[1]*cfg.MODEL.DOWNSAMPLE*h_ratio)
    ...
    # val_loader = get_val_dataset(image_dir,anno_file,vis_dir,model,preprocess)
